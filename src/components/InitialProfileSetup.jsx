...
export default function InitialProfileSetup() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      try {
        const { data: bucketData, error: bucketCheckError } =
          await supabase.storage.getBucket("profile-images");
      ...
      const normalizedFirstName = formData.firstName
        .toLowerCase()
        .replace(/[^a-z0-9]/g, "");
      const normalizedLastName = formData.lastName
        .toLowerCase()
        .replace(/[^a-z0-9]/g, "");
      const username = `${normalizedFirstName}-${normalizedLastName}-${Math.floor(
        Math.random() * 1000
      )}`;
      ...
      if (isFreelancer) {
        ...
        if (freelancerProfileError) {
          console.error(
            "Freelancer profile creation error:",
            freelancerProfileError
          );

          // If the table doesn't exist, try to redirect to setup
          if (
            freelancerProfileError.message &&
          ...
          toast.error(
            "Could not create freelancer profile: " +
              freelancerProfileError.message
          );
        } else {
          freelancerUsername = freelancerProfile?.username || username;
          console.log(
            "Freelancer profile created with username:",
            freelancerUsername
          );
        }
        ...
        if (formData.university) {
          ...
          if (educationError) {
            ...
            toast.error(
              "Could not save education details: " + educationError.message
            );
          }
        }
      ...
      if (isFreelancer) {
        const profileUrl = `/freelancer/${
          freelancerUsername || username || user.id
        }`;
          ...
          <form onSubmit={(e) => e.preventDefault()}>
              ...
              >
                <div className="space-y-6">
                  <div className="bg-blue-50 rounded-lg p-6 mb-6">
                    ...
                    <p className="text-blue-700">
                      Showcase your abilities and connect your online presence
                    </p>
                  </div>
            ...
            <div className="mt-8 flex justify-between">
              ...
              <motion.button
                ...
                } px-6 py-3 bg-gradient-to-r from-blue-600 to-blue-700 text-white rounded-lg hover:from-blue-700 hover:to-blue-800 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:opacity-50 shadow-lg`}
              >
                {loading ? (
